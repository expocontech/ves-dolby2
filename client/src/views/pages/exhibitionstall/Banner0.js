...
class Banner extends React.Component {
  ...
  componentDidMount = () => {
    if (sessionStorage.getItem('token') != undefined) {
      ...
      axios.get(`${process.env.REACT_APP_BASENAME}exhibition/get-stall/${ex_hallid}`, { headers: authHeader }).then((response) => {
    ...
    return (
      ...
      <Row>
        ...
        {this.state.data.map(item => (
          <div id="stalldetails" className="stalldetails" style={{ position: "absolute", left: item.dleft + "%", width: item.dwidth + "%", height: item.dheight + "%", top: item.dtop + "%", cursor: "pointer", WebkitTransform: "rotate(0deg)" }}
            onClick={() => { history.push(`/pages/sponsordetails/${item.sponsorid}`) }}>
